    ...
    DataTable.advanced.init = function(settings) {
        ...
        $.extend(DataTable.defaults, {
            ...
            'columnDefs': [{
                ...
                'targets': 'image', // First column from the right,                
                'render': function(data, type, row, meta){ 
                	
					if(data!=null&&data.startsWith("data:image")){
						 	var datatables = new $.fn.dataTable.Api(meta.settings);
					        var showUrl = getShowUrl(datatables, row.id);
						return  "<a href=\""+showUrl+"\"><img id=\"image\" src=\""+data+"\" class=\"rounded img-thumbnail\" alt=\"Smiley face\" height=\"42\" width=\"42\"/></a>";
					}
                    return data;
                }
            }, {
                'targets': 'download', // First column from the right,                
                'render': function(data, type, row, meta){
                	//console.log(meta);                	
					if(data!=null){
						
						return  "<a href=\"/downloadFile/"+data+"\">Download</a>";
