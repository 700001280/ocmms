...
public class EmbeddableImage {
   ...
   * @return Image the same image but resized to the specified dimensions.
   */
  public EmbeddableImage resize(int width) {
    ...
    try {
      ...
      int oWidth=imageBuffer.getWidth();
      int oHeight=imageBuffer.getHeight();
      int nHeight=oHeight*width/oWidth;
      // Creating new BufferedImage with the new resized size
      BufferedImage resizedImage = new BufferedImage(width, nHeight, type);

      // Creating graphics that will create the resize image
      Graphics2D g = resizedImage.createGraphics();
      g.drawImage(imageBuffer, 0, 0, width, nHeight, null);
      g.dispose();
