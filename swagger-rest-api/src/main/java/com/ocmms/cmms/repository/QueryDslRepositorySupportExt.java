...
/**
 * Base class with additional utilities for implementing repositories using
 * QueryDsl library.
 *
...
public class QueryDslRepositorySupportExt<T> extends QuerydslRepositorySupport {
	...
	/**
	 * Applies the given {@link Pageable} to the given {@link JPQLQuery}. Allows to
	 * map the attributes to order as provided in the {@link Pageable} to real
	 * entity attributes. This might be used to work with projections or DTOs whose
	 * attributes don't have the same name as the entity ones.
	 *
	 * It allows to map to more than one entity attribute. As an example, if the DTO
	 * used to create the {@link Pageable} has a fullName attribute, you could map
	 * that attribute to two entity attributes: name and surname. In this case, the
	 * {@link Pageable} defines an order by a fullName attribute, but que query will
	 * order by name and surname instead.
	 *
	 * @param pageable the ordering and paging
	 * @param query
	 * @param mapping  definition of a mapping of order attribute names to real
	 *                 entity ones
	 * @return the updated query
	 */
	 ...
	 * @param pageable         the ordering and paging
	 * @param query
	 * @param attributeMapping definition of a mapping of order attribute names to
	 *                         real entity ones
	 * @return the updated query
	 */
	protected JPQLQuery<T> applyPagination(Pageable pageable, JPQLQuery<T> query,
		...
		if (sort != null) {
			...
			for (Sort.Order order : sort) {
				if (!attributeMapping.containsKey(order.getProperty())) {
					LOG.warn("The property (%1) is not included in the attributeMapping, will order "
							+ "using the property as it is", order.getProperty());
			...
			if (mappedOrders.isEmpty()) {
				// No properties to order by are available, so don't apply ordering and return
				// the query
				// as it is
				return query;
			}
			mappedPageable = PageRequest.of(pageable.getPageNumber(), pageable.getPageSize(), Sort.by(mappedOrders));
	...
	/**
	 * Creates a factory to easily build an attribute mapping to real entity
	 * attribute names.
	 * 
	 ...
	 * applied, so you have always an absolute order. Otherwise, the order of the
	 * results depends on the database criteria, which might change even between
	 * pages, returning confusing results for the user.
	 * 
	 * @param query
	 * @return the updated query
	 */
	...
	/**
	 * Adds a global contains text filter on the provided attributes. WARNING: this
	 * creates a very inefficient query. If you have many entity instances to query,
	 * use instead an indexed text search solution for better performance.
	 * 
	 * @param text                   the text to look for
	 * @param query
	 * @param globalSearchAttributes the list of attributes to perform the filter on
	 * @return the updated query
	 */
	...
	/**
	 * Loads a page of data with the provided pagination criteria. It allows to load
	 * full entities as well as projections.
	 *
	 * TODO: the current implementation expects the query to have already applied
	 * the paging and sorting criteria, which is not what one could expect from the
	 * method signature.
	 *
	 ...
	 *
	 * <pre class="code">
	 * loadPage(query, pageable,
	 * 		Projections.constructor(EmployeeInfo.class, employee.id, employee.firstName, employee.lastName,
	 * 				employee.phone, employee.extension, employee.supervisor.id, employee.supervisor.firstName,
	 * 				employee.supervisor.lastName));
	 * </pre>
	 *
	 * @param <M>        the data type to load, usually a JPA Entity or a projection
	 *                   bean
	 * @param query      the query with the pagination and ordering criteria already
	 *                   applied
	...
	/**
	 * Factory to create a mapping between simulated attributes and the entity real
	 * ones.
	 */
	public static class AttributeMappingBuilder {
		...
		/**
		 * Adds a mapping between a virtual attribute and one or more real ones. As an
		 * example, if there is a DTO with a {@code fullName} attribute, you could map
		 * it to the entity {@code name} and {@code surname} ones.
		 * 
